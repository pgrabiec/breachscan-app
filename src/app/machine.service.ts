...
export class MachineService {
  ...
  getMachine(address: string): Observable<Machine> {
    // Todo: send the message _after_ fetching the machine
    this.messageService.add(`MachineService: fetched machine address=${address}`);
    return of(MACHINES.find(machine => machine.address === address));
  }
}
